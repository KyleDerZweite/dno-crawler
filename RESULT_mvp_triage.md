...
- **File**: `backend/app/core/auth.py`, lines 270-280
...
- **File**: `backend/app/jobs/search_job.py` (entire file)
- **File**: `backend/app/jobs/__init__.py`, lines 89, 163
...
- **File**: `backend/app/services/discovery/sitemap.py`, lines 97, 100, 101
...
- **Files**: `backend/app/services/extraction/` (directory)
...
- **Implementation Target**: Add `backend/app/services/extraction/xlsx_extractor.py` and wire it into the extraction pipeline entrypoint in `backend/app/services/extraction/` (classifier/dispatch stage).
...
- **File**: `frontend/src/features/dno-detail/views/SQLExplorer.tsx`, line 198
...
- **File**: `frontend/src/features/dno-detail/views/Technical.tsx`, line 128
...
- **File**: `frontend/src/features/dno-detail/views/Tools.tsx`
...
- **File**: `backend/app/services/ai/providers/litellm.py`
...
- **File**: `backend/app/services/impressum_extractor.py`, lines 18-24
...
- **File**: `frontend/src/features/dno-detail/components/PriceTrendChart.tsx`, line 55
...
- **File**: `backend/scripts/recheck_robots.py`
...
- **Files**: `frontend/src/App.test.tsx` (only test file)
...
- **File**: `frontend/src/pages/DNOsPage.tsx`, line 289
...
|------|------|-----------------|-----------------|
| 1 | C1: Add `AI_ENCRYPTION_KEY` to `.env.example` — Resolved in PR `#6` | 0 min | Startup crash / unencrypted secrets |
| 2 | C2: Add `TRUSTED_PROXY_COUNT` to `.env.example` — Resolved in PR `#6` | 0 min | Rate limiting bypass |
| 3 | C3: Fail-fast startup check for auth in production — Resolved in PR `#6` | 0 min | Confusing partial availability |
| 4 | C4: Remove legacy `search_job.py` and `WorkerSettings` — Resolved in PR `#6` | 0 min | Silent data pipeline failure |
| 5 | C5: Fix `type: ignore` in sitemap service — Resolved in PR `#6` | 0 min | Potential runtime crash during crawl |
| 6 | C6: XLSX extractor | Deferred post-MVP | Track as extraction expansion item |

**Total estimated outstanding effort for Critical MVP launch items: 0 min (C1-C5 completed; C6 deferred post-MVP).**
