/**
 * SourceDataAccordion - Collapsible section showing external data sources
 * Displays MaStR, VNB Digital, and BDEW data when available
 */

import { useState } from "react";
import { Badge } from "@/components/ui/badge";
import { Database, Globe, Building, Info, ChevronUp, ChevronDown } from "lucide-react";
...
export function SourceDataAccordion({
    hasMastr,
    hasVnb,
    hasBdew,
    mastrData,
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                    ...
                    className={cn("text-xs", hasMastr && "bg-blue-500")}
                >
                    MaStR {hasMastr ? "✓" : "–"}
                </Badge>
                <Badge
                    variant={hasVnb ? "default" : "outline"}
                    className={cn("text-xs", hasVnb && "bg-green-500")}
                >
                    VNB Digital {hasVnb ? "✓" : "–"}
                </Badge>
                <Badge
                    variant={hasBdew ? "default" : "outline"}
                    className={cn("text-xs", hasBdew && "bg-purple-500")}
                >
                    BDEW {hasBdew ? "✓" : "–"}
                </Badge>
            </div>
            ...
            >
                ...
                <span>{expanded ? "Hide Details" : "Show External Data"}</span>
                {expanded ? (
