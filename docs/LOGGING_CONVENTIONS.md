...
| Problem | Description |
|---------|-------------|
| Multiple log lines | 6 log lines for one request |
| No correlation | Lines are not connected to each other |
| Missing context | Which user? What amount? Why did it fail? |
| Hard to query | Impossible to query effectively |
...
These logs are designed to answer questions like the following.

| Question | Query |
|----------|-------|
| Show me all failed requests for premium users in the last hour | `WHERE outcome = 'error' AND user.subscription = 'premium' AND timestamp > now() - 1h` |
| What is the error rate by endpoint? | `GROUP BY http.path, COUNT(*) WHERE outcome = 'error' / COUNT(*)` |
| Find the slowest database queries | `WHERE db.duration_ms > 1000 ORDER BY db.duration_ms DESC` |
| Correlate this user journey across services | `WHERE user.id = 'user_456' ORDER BY timestamp` |
...
| Format | Status |
|--------|--------|
| `2026-01-09T10:23:47.123Z` | Correct |
| `1704792227` | Incorrect (Unix timestamp) |
| `Jan 9 2026 10:23:47` | Incorrect (human readable only) |
| `2026-01-09 10:23:47` | Incorrect (missing timezone) |
...
user_id, request_id, created_at, error_message   # Correct
userId, RequestId, createdAt, errorMessage        # Incorrect

# Use dot notation for nested objects
user.id, user.email, http.method, error.code     # Correct
user_id (when there is also user_email)           # Incorrect

# Use lowercase for enum values
"status": "pending", "level": "error"             # Correct
"status": "PENDING", "level": "ERROR"             # Incorrect

# Boolean fields should be prefixed with is_, has_, can_
is_admin, has_subscription, can_edit              # Correct
admin, subscribed, editable                       # Incorrect
```
...
| Slow requests (greater than 2s) | 100% | Performance issues |
...
def should_sample(event: dict) -> bool:
    ...
    # ALWAYS keep slow requests (greater than 2s)
...
|----------|-------------|
| Emit one wide event per request | Complete context in a single log entry |
| Use consistent field names (snake_case) | Enables pattern matching and querying |
| Include request_id in every event | Enables request correlation |
| Add business context | User tier, feature flags, etc. |
| Keep errors at 100% sample rate | Never miss failure information |
| Use structured JSON format | Machine parseable |
| Use ISO 8601 timestamps with timezone | Unambiguous time representation |
...
| String interpolation for log messages | Use structured fields instead |
| Emitting multiple logs for one request | Use wide events |
| Logging inside tight loops | Performance impact |
| Inconsistent field names | Breaks querying |
| Forgetting error context | Makes debugging difficult |
| Logging without request_id | Breaks correlation |
