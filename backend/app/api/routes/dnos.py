...
VALID_VOLTAGE_LEVELS = ["HS", "HS/MS", "MS", "MS/NS", "NS"]
...
class NetzentgelteImport(BaseModel):
    ...
    def validate_voltage_level(cls, v: str) -> str:
        if v not in VALID_VOLTAGE_LEVELS:
            raise ValueError(f"Invalid voltage level. Must be one of: {VALID_VOLTAGE_LEVELS}")
        return v
    
    @field_validator("verification_status")
    @classmethod
    def validate_verification_status(cls, v: str | None) -> str | None:
        if v is not None and v not in ["unverified", "verified", "flagged"]:
...
class ImportRequest(BaseModel):
    ...
    def validate_record_count(cls, v: list) -> list:
        if len(v) > MAX_IMPORT_RECORDS:
            raise ValueError(f"Maximum {MAX_IMPORT_RECORDS} records allowed per import")
        return v
