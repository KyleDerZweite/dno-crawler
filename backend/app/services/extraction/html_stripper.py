...
class HtmlStripper:
    ...
    def strip_html(self, html: str) -> tuple[str, list[int]]:
            ...
            if year_match:
                ...
                if table:
                    ...
                    essential_content.append(
                        {"year": year, "header": str(clean_header), "table": str(clean_table)}
                    )
        ...
        if not essential_content:
            for table in soup.find_all("table"):
                clean_table = self._clean_element(table)
                essential_content.append({"year": None, "header": "", "table": str(clean_table)})
    ...
    def strip_and_split(
        self, html_content: str, output_dir: Path, slug: str, data_type: str
    ) -> list[tuple[int, Path]]:
        """
