...
from app.services.web_crawler import WebCrawler, get_keywords_for_data_type, NEGATIVE_KEYWORDS
            ...
            if sitemap_found and sitemap_results:
                ...
                if data_type == "hlzf":
                    ...
                    if not hlzf_specific:
                        print(f"   ⚠️ Sitemap has no HLZF-specific files, will also check HTML pages")
                else:
                ...
                if data_type == "hlzf" and not hlzf_specific:
                    print(f"   🔍 Doing BFS scan to find HTML pages with embedded HLZF data...")
                    await _scan_for_hlzf_html(
                        client, start_url, target_year, all_documents, verbose
                    )
            elif sitemap_found:
                print(f"   ⚠️ Sitemap found but no relevant URLs")
            else:
...
def print_results(results: dict, verbose: bool):
    ...
    if relevant_docs:
        ...
        print(f"❌ No documents discovered - site may use JavaScript or different structure")
